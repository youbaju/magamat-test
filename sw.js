/*
 * Service Worker لموقع "عود و مقامات" (East-West Maqamat)
 * ==========================================================
 * الاستراتيجية المتبعة (بعد الموازنة بين حجم التخزين وتجربة الأوفلاين):
 *
 * 1) هيكل الصفحة (index.html): "الشبكة أولاً" (Network-First) — يحاول يجيب
 *    أحدث نسخة من الإنترنت في كل زيارة، ويحدّث الكاش تلقائياً. لو ما فيه نت
 *    يرجع للنسخة المخزنة آخر مرة. هذا يضمن أي تحديث مستقبلي على محتوى الموقع
 *    ينعكس فوراً بدون أي حاجة نتذكر نرفع رقم CACHE_VERSION يدوياً.
 *
 * 2) PRECACHE (تخزين فوري عند أول زيارة، كاش-أولاً لأنها ملفات ثابتة):
 *    - المانيفست + الأيقونات
 ...
 *
 * 3) RUNTIME CACHE (تخزين تدريجي عند أول استخدام فعلي):
 ...
 *
 * ملاحظة: لو غيّرت أسماء أو مسارات ملفات PRECACHE_ASSETS نفسها (نادراً ما
 * يصير)، ارفع رقم CACHE_VERSION بالأسفل عشان تتولد نسخة كاش جديدة نظيفة.
...
self.addEventListener('fetch', (event) => {
  ...
  // هيكل الصفحة الرئيسي (index.html): "الشبكة أولاً" (Network-First) بدل الكاش أولاً.
  // هذا مهم جداً: index.html هو الملف اللي فيه كل منطق ومزايا الموقع، وبيتغيّر
  // بانتظام مع أي تحديث مستقبلي. لو استخدمنا كاش-أولاً هنا (زي باقي الملفات)،
  // أي مستخدم زار الموقع قبل بيضل عالق على نسخة قديمة للأبد حتى لو رفعنا تحديثات
  // جديدة على السيرفر، لأن الكاش يمنع أي طلب شبكة أصلاً. بالطريقة هذي: يحاول
  // يجيب أحدث نسخة من الإنترنت أول شي (ويحدّث الكاش بنفس الوقت)، ولو ما فيه نت
  // يرجع فوراً للنسخة المخزنة آخر مرة — نتيجة النهاية: تحديثات المحتوى تنعكس
  // تلقائياً بدون أي حاجة نتذكر نرفع رقم CACHE_VERSION يدوياً في كل مرة.
...
// شبكة-أولاً لملف الصفحة الرئيسي: يضمن كل تحديث محتوى ينعكس فوراً لأي زائر
// متصل بالنت، مع رجوع تلقائي للنسخة المخزنة لو قطع النت (يشتغل أوفلاين برضه)
...
// كاش-أولاً لملفات PRECACHE الثابتة (سريع، وهذي الملفات ما تتغير كثير)
